...
help them, and - since you'll be promoting efficiency at the same time - 
increase your chances of a promotion. 

Braille is a writing system used to read by touch instead of by sight. 
Each character is composed of 6 dots in a 2x3 grid, where each dot can either 
be a bump or be flat (no bump). You plan to translate the signs around the 
space station to Braille so that the minions under Commander Lambda's command 
can feel the bumps on the signs and "read" the text with their touch. 
The special printer which can print the bumps onto the signs expects the dots 
...
Output:
    (string) "000001011110110010100010000000111110101001010100100100101000000000
    1100001110101010100101111011100000001101001010101011010000000101101010011011
    0011110010001010011000000010101011100110001011101000000001111011001010001000
    0000111000100000101011101111000000100110101010110110"
...
def build_bra_map():
    bra_map = {CAP_MARK: None}

    input = 'The quick brown fox jumped over the lazy dog'
    letters = []
    for letter in split(input, 1):
        if letter.isupper():
            letters.extend([CAP_MARK, letter.lower()])
        else:
            letters.append(letter)

    output = '00000101111011001010001000000011111010100101010010010010100000' \
             '00001100001110101010100101111011100000001101001010101011010000' \
             '00010110101001101100111100100010100110000000101010111001100010' \
             '11101000000001111011001010001000000011100010000010101110111100' \
             '0000100110101010110110'
    for letter, code in zip(letters, split(output, 6)):
        bra_map[letter] = code
    return bra_map
...
def answer(plaintext):
    ...
    for char in plaintext:
        if char.isupper():
            ret += BRA_MAP.get(CAP_MARK)
        ret += BRA_MAP.get(char.lower())
    return ret
